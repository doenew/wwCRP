...
unifullD108 <- extract_and_combine(result_list, 3)
unifullD208 <- extract_and_combine(result_list, 4)
unifullMPD  <- extract_and_combine(result_list, 5)
unifullFFFD <- extract_and_combine(result_list, 6)

for(j in 1:5){
  results50=data.frame(D105=unifullD105[,j],
                       D205= unifullD205[,j],
                       D108=unifullD108[,j],
                       D208 =unifullD208[,j],
                       MPD = unifullMPD[,j],
                       FFFD=unifullFFFD[,j])
  boxplot( results50)
  write.csv(results50,paste0(base_path, "re50", j, ".csv"))
}
...
for(j in 1:5){
  results100=data.frame(D105=unifullD105[,j],
                       D205= unifullD205[,j],
                       D108=unifullD108[,j],
                       D208 =unifullD208[,j],
                       MPD = unifullMPD[,j],
                       FFFD=unifullFFFD[,j])
  boxplot( results100)
  write.csv(results100,paste0(base_path, "re100", j, ".csv"))
}
